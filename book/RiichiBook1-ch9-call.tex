...
\chapter{Les appels (réquisitions)} \label{ch:call}
\thispagestyle{empty}
...
\bigskip
\section{Choix d'appels : exemples}
...
\subsection{Éliminer les mauvaises attentes}
...
\subsection{Améliorer l'attente}
...
\bigskip
\subsection{Confirmer son {\jap yaku}}
...
\subsection{Brelan caché de valeur}
...
%\bigskip
\begin{itembox}[r]{Brelan caché de valeur}
\bp
\wan{4}\wan{6}\wan{8}\tong{2}\tong{4}\suo{1}\suo{2}\suo{5}\suo{6}\suo{6}\zhong\zhong\zhong~~\xi\\
\hfill\footnotesize{{\jap Dora}~~~~~~~~~~}
\ep
\vspace{-20pt}Le joueur à gauche vient d'écarter {\LARGE\wan{5}}.
\end{itembox}
...
\subsubsection{Quand ne pas faire un carré caché}
...
\newpage
\section{Astuces pour les appels}
...
\begin{itembox}[r]{Utiliser les {\jap dora}}
\bp
...
\subsection{Soyez attentif à l’emplacement des joueurs}
